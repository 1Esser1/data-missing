...
import { Link2, Unlink, Move, Loader2, X, GitMerge, RefreshCw, Users, User, Brain, LayoutTemplate } from 'lucide-react';
...
/** Topological layout using Kahn's algorithm — returns { taskId: { x, y } } */
function computeTopologicalLayout(tasks, links) {
  ...
  // Nodes in cycles or isolated get their own column
...
export default function TaskRelations() {
  ...
  const [mode,        setMode]        = useState('move');   // 'move' | 'link'
  const [linkStart,   setLinkStart]   = useState(null);     // first selected taskId
  const [linkPending, setLinkPending] = useState(null);     // { fromId, toId } — awaiting direction choice
  const [hovered,     setHovered]     = useState(null);
  const [linkTooltip, setLinkTooltip] = useState(null); // { x, y, reason, fromTitle, toTitle }
  ...
  const onNodeClick = (id) => {
    ...
    // Show direction picker instead of immediately linking
  ...
  return (
    <PageWrapper title="Task Relations" subtitle="Drag to organize · Link mode to define dependencies · Arrow = prerequisite → dependent">
      {/* Toolbar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
        <button
          onClick={() => { setMode('move'); setLinkStart(null); setLinkPending(null); }}
          style={{
            display: 'inline-flex', alignItems: 'center', gap: '6px',
            padding: '7px 14px', borderRadius: '8px',
            border: `1.5px solid ${mode === 'move' ? '#1A1A2E' : theme.borderMed}`,
            background: mode === 'move' ? '#1A1A2E' : theme.cardBg,
            color: mode === 'move' ? 'white' : theme.textMed,
        ...
        <button
          onClick={() => { setMode('link'); setLinkStart(null); setLinkPending(null); }}
          style={{
            display: 'inline-flex', alignItems: 'center', gap: '6px',
            padding: '7px 14px', borderRadius: '8px',
            border: `1.5px solid ${mode === 'link' ? '#7C3AED' : theme.borderMed}`,
            background: mode === 'link' ? '#7C3AED' : theme.cardBg,
            color: mode === 'link' ? 'white' : theme.textMed,
            fontSize: '0.8rem', fontWeight: '600', cursor: 'pointer',
          }}
        >
          <Link2 size={14} /> Link Mode
        </button>
        ...
        {isPrivileged && devList.length > 0 && (
          <div style={{
            ...
            padding: '4px 8px', border: `1px solid ${theme.border}`, borderRadius: '10px', background: theme.hoverBg,
          }}>
        ...
        {mode === 'link' && !linkPending && (
          <div style={{
            ...
            {linkStart
              ? `Selected: "${tasks.find(t => t.id === linkStart)?.title?.slice(0, 30)}…" — click another task`
              : 'Click a task node to start linking'}
          </div>
        )}
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div
            ...
            onMouseUp={onMouseUp}
            onMouseLeave={() => { onMouseUp(); setLinkTooltip(null); }}
          >
            {/* AI reason tooltip */}
            {linkTooltip && (
              <div style={{
                position: 'absolute',
                left: linkTooltip.x + 16,
                top: linkTooltip.y - 36,
                background: '#1A1A2E',
                color: 'white',
                padding: '8px 12px',
                borderRadius: '8px',
            ...
            <svg style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }} width={canvasWidth} height={canvasHeight}>
              ...
              {visibleLinks.map(l => {
                ...
                // Shorten the line so it ends at the node edge, not center
                ...
                const color = l.aiDefined ? '#059669' : '#7C3AED';
                const markerId = l.aiDefined ? 'arrowhead-ai' : 'arrowhead';

                return (
                  <g key={`${l.fromTaskId}-${l.toTaskId}`}>
                    <line
                      x1={from.x} y1={from.y} x2={endX} y2={endY}
                      stroke={color} strokeWidth="2" strokeDasharray="5,3"
                      opacity="0.6" markerEnd={`url(#${markerId})`}
                    />
                    {l.sequenceOrder && (
                    ...
                    {/* AI reason hover zone — invisible wider hit area on the line */}
                    {l.aiDefined && l.aiReason && (
                      <line
                        x1={from.x} y1={from.y} x2={endX} y2={endY}
                        stroke="transparent" strokeWidth="16" style={{ pointerEvents: 'all', cursor: 'help' }}
                        onMouseEnter={() => setLinkTooltip({ x: mx, y: my, reason: l.aiReason, fromTitle: l.fromTitle, toTitle: l.toTitle })}
                    ...
                    <g
                      ...
                      onMouseEnter={() => setHovered(`${l.fromTaskId}-${l.toTaskId}`)}
                      onMouseLeave={() => setHovered(null)}
                    >
                      <circle cx={mx} cy={my} r={10} fill={theme.cardBg} stroke="#DDD6FE" strokeWidth="1.5" />
                      <text x={mx} y={my + 4} textAnchor="middle" fontSize="11" fill={color}>×</text>
                    </g>
                    {hovered === `${l.fromTaskId}-${l.toTaskId}` && (
            ...
            {visibleTasks.map(task => {
              ...
              const borderColor = isSelected || isPendingA || isPendingB
                ? '#7C3AED'
                : isLinked ? '#DDD6FE' : '#E5E7EB';
              const borderWidth = isSelected || isPendingA || isPendingB ? '2px' : '1.5px';
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    {isLinked && (
                      <GitMerge size={10} color="#7C3AED" style={{ marginLeft: 'auto' }} />
                    )}
                  </div>
      ...
      {/* Legend */}
      <div style={{ display: 'flex', gap: '1.5rem', marginTop: '0.75rem', fontSize: '0.72rem', color: theme.textMuted, flexWrap: 'wrap' }}>
        ...
        <span>· Arrow points FROM prerequisite TO dependent · Click × on a line to remove</span>
      </div>
